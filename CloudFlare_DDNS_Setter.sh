...
lightsail_change_ip(){
    #检查本机ip是否被tcp阻断
    tcp_status=`curl --silent --connect-timeout 5 http://47.103.53.182:10535/${local_ip}/11443 | jq .status`
...
azure_change_ip(){
    #检查本机ip是否被tcp阻断
    tcp_status=`curl --silent http://47.103.53.182:10535/${local_ip}/11443 | jq .status`
