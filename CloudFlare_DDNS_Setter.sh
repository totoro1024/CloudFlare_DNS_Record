...
choose_service(){
	...
	[[ "${service}" = "4" ]] && Azure_conf

	elif [[ "$1" == "--ddns" ]]; then
        if [[ $lightsail_switch == "true" ]]; then
            lightsail_change_ip
        fi
	if [[ $azure_switch == "true" ]]; then
            azure_change_ip
        fi
        check_ip_diff
...
lightsail_change_ip(){
    #检查本机ip是否被tcp阻断
    tcp_status=`curl --silent --connect-timeout 5 http://47.103.53.182:10535/ping/${local_ip}/11443 | jq .status`
...
azure_change_ip(){
    #检查本机ip是否被tcp阻断
    tcp_status=`curl --silent http://47.103.53.182:10535/ping/${local_ip}/11443 | jq .status`
